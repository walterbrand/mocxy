...
function findCsvFilesRecursively(dir: string, baseDir: string): string[] {
...
function expandDottedKeys(record: Record<string, any>): Record<string, any> {
...
export function loadCsvDataAsMap(basePath: string): Map<string, object[]> {
    ...
    for (const relativePath of csvFiles) {
        ...
        try {
            ...
            const cleaned = records.map((record: Record<string, any>) => {
                const result: Record<string, any> = {};
                for (const [key, value] of Object.entries(record)) {
                    if (value !== undefined) {
                        result[key] = value;
